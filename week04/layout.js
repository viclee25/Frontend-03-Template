function getStyle(element) {
  ...
  for (const prop of element.computedStyle) {
...
function layout(element) {
  ...
  var mainSpace = elementStyle[mainStyle];
  var crossSpace = 0;
  ...
  flexLines.forEach((items) => {
    var lineCrossSize = style.alignContent === 'stretch' ? item.crossSpace + flexLines.length : item.crossSpace;
    for (let i = 0; i < items.length; i++) {
      var item = items[i];
      var itemStyle = getStyle(item);
